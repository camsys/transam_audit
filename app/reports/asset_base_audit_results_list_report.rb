class AssetBaseAuditResultsListReport < Report
  def get_data(conditions)
    ...
    if conditions[:audit_result_type_id].to_i == AuditResultType::AUDIT_RESULT_UNKNOWN.to_i
      ...
      not_tested = TransitAsset.operational.where(organization_id: conditions[:organization_id], filterable_id: conditions[:filterable_id]).where.not(id: AuditResult.where(conditions.except(:audit_result_type_id)).pluck(:auditable_id))
