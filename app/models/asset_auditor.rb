...
class AssetAuditor < AbstractAuditor
  ...
  def update_status a, start_date, end_date
    ...
    audit_result = AuditResult.find_or_initialize_by(:organization_id => asset.organization_id, :auditable_id => a.id, :auditable_type => Rails.application.config.asset_base_class_name, :audit_id => context.id, :filterable_type => "FtaAssetCategory", :filterable_id => asset.fta_asset_category)
