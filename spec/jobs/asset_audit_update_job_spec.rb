...
RSpec.describe AuditableUpdateJob, :type => :job do
  ...
  it '.run' do
    ...
    AuditableUpdateJob.new(test_audit, test_asset.object_key).run
  ...
  it '.prepare' do
    ...
    expect(Rails.logger).to receive(:debug).with("Executing AuditableUpdateJob at #{Time.now.to_s} for Audit #{test_audit} and Asset #{test_asset.object_key}")
